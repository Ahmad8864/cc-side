...
const Composer: ClientModule<ComposerProps, State> = (props, surface) => {
  ...
  const redraw = () => { surface.setState({ ...instance.state }); snapshot(instance, surface) }
  const setText = (text: string) => {
    instance.state = { ...instance.state, text, cursor: text.length, selected: 0, hiddenMenu: false, preferredColumn: undefined }
    instance.state.seq++; redraw()
  }
  instance.send = () => {
  ...
  surface.onKey(key => {
    ...
    const menu = completions(state.text, instance.props.commands, instance.props.models, instance.props.model)
    const shown = state.hiddenMenu ? [] : menu
    if (key.ctrl && key.key === 'g') { state.hiddenMenu = true; redraw(); return }
    if (shown.length && (key.key === 'up' || key.key === 'down')) {
      state.selected = (state.selected + (key.key === 'up' ? -1 : 1) + shown.length) % shown.length
      redraw(); return
    }
    if (shown.length && key.key === 'tab') { choose(shown[state.selected % shown.length], false); return }
    ...
    if (!shown.length && !state.text.includes('\n') && (key.key === 'up' || key.key === 'down') && (state.historyIndex >= 0 || key.key === 'up' && state.cursor === 0)) {
      if (state.historyIndex < 0) { state.unsent = state.text; state.historyIndex = state.history.length }
      const index = Math.max(0, Math.min(state.history.length, state.historyIndex + (key.key === 'up' ? -1 : 1)))
      state.historyIndex = index
      setText(index === state.history.length ? state.unsent : state.history[index] ?? '')
      return
    }
    const next = edit(state, key, width)
    if (next.text !== state.text) { state.selected = 0; state.hiddenMenu = false; state.historyIndex = -1 }
    instance.state = { ...state, ...next, seq: state.seq + 1 }
    redraw()
  })
  const state = io.state
  const lines = layout(state.text, width), position = caret(lines, state.cursor)
  ...
  return <Box flexDirection="column" width={width + 3}>
    ...
    {lines.slice(start, start + count).map((line, index) => {
      ...
      const before = line.glyphs.filter(g => g.end <= state.cursor).map(g => g.text).join('')
    ...
    <Text dimColor>{rule}</Text>
    {instance.menu.slice(menuStart, menuStart + menuSize).map((item, index) => <Box key={item.value} height={1}>
      <Text color={menuStart + index === state.selected ? 'claude' : undefined} bold={menuStart + index === state.selected} wrap="truncate-end">

...
export const localCommands: SideCommand[] = [
  ...
  { name: 'effort', description: 'Set thinking effort for this side chat', argumentHint: '[level]' },
...
export function commandCatalog(commands: SideCommand[]): SideCommand[] {
  ...
  return [...localCommands, ...commands.filter(c => !reserved.has(c.name))]
    .map(c => ({ name: c.name, description: c.description.slice(0, 180), argumentHint: c.argumentHint, ...(c.aliases ? { aliases: c.aliases } : {}) }))
}
...
export function modelLabel(model: string, models: SideModel[] = []) {
  ...
  return entry?.description.split(' · ')[0] || model.replace(/^claude-/, '').replace(/\[1m\]/, ' (1M context)').replace(/-/g, ' ')
}

export type Completion = { value: string; label: string; description: string; execute?: boolean }
export function completions(text: string, commands: SideCommand[], models: SideModel[], model?: string): Completion[] {
  ...
  if (match) {
    ...
    return [...levels, 'auto'].filter(level => level.startsWith(query))
      .map(level => ({ value: `/effort ${level}`, label: level, description: level === 'auto' ? 'Use the model default' : 'This side chat only', execute: true }))
  }
  if (/\s/.test(text)) return []
  const query = text.slice(1).toLowerCase()
  return commands.filter(c => c.name.startsWith(query) || c.aliases?.some(a => a.startsWith(query)))
    .map(c => ({ value: `/${c.name}${c.argumentHint ? ' ' : ''}`, label: `/${c.name}`, description: c.description }))
}

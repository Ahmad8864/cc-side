...
export function glyphs(text: string): Glyph[] {
  const segments = typeof Intl !== 'undefined' && 'Segmenter' in Intl
    ...
    : Array.from(text).reduce<string[]>((out, ch) => {
      if (out.length && (/\p{Mark}|\uFE0F|\u200D/u.test(ch) || out.at(-1)!.endsWith('\u200D'))) out[out.length - 1] += ch
      else out.push(ch)
      return out
    }, [])
  let start = 0
  return segments.map(value => {
    ...
    const width = value === '\n' ? 0 : /\p{Extended_Pictographic}|\p{Regional_Indicator}/u.test(value)
      || point >= 0x1100 && (point <= 0x115f || point >= 0x2e80 && point <= 0xa4cf || point >= 0xac00 && point <= 0xd7a3 || point >= 0xf900 && point <= 0xfaff || point >= 0xfe10 && point <= 0xfe6f || point >= 0xff01 && point <= 0xff60 || point >= 0xffe0 && point <= 0xffe6 || point >= 0x20000) ? 2 : 1
...
export function layout(text: string, columns: number): Line[] {
  ...
  const result: Line[] = []
  let row: Glyph[] = [], width = 0, start = 0
  const push = (end: number) => { result.push({ start, end, glyphs: row, width }); row = []; width = 0; start = end }
  for (const glyph of glyphs(text)) {
    if (glyph.text === '\n') { push(glyph.start); start = glyph.end; continue }
    if (width + glyph.width > limit) {
      const space = row.findLastIndex(g => /\s/.test(g.text))
      const rest = row.slice(space + 1)
      if (space >= 0 && space < row.length - 1 && rest.reduce((n, g) => n + g.width, 0) + glyph.width <= limit) {
        row = row.slice(0, space + 1); width = row.reduce((n, g) => n + g.width, 0)
        push(rest[0].start); row = rest; width = rest.reduce((n, g) => n + g.width, 0)
      } else push(glyph.start)
    }
    row.push(glyph); width += glyph.width
  }
  push(text.length)
  // Give the caret a cell at the end of an exactly full line.
  if (result.at(-1)!.width === limit) result.push({ start: text.length, end: text.length, glyphs: [], width: 0 })
  return result
}
...
export function caret(lines: Line[], cursor: number) {
  ...
  row = Math.max(0, row)
  return { row, column: lines[row].glyphs.filter(g => g.end <= cursor).reduce((n, g) => n + g.width, 0) }
}
...
export function edit(state: Editor, key: Key, columns: number): Editor {
  ...
  const replace = (start: number, end: number, insert = ''): Editor => ({ text: text.slice(0, start) + insert + text.slice(end), cursor: start + insert.length })
  ...
  if (key.key === 'home' || key.ctrl && key.key === 'a') return move(key.ctrl && key.key === 'home' ? 0 : lineStart)
  if (key.key === 'end' || key.ctrl && key.key === 'e') return move(key.ctrl && key.key === 'end' ? text.length : lineEnd)
  if (key.key === 'up' || key.key === 'down') {
    const lines = layout(text, columns), position = caret(lines, cursor)
    const column = state.preferredColumn ?? position.column
    const target = Math.max(0, Math.min(lines.length - 1, position.row + (key.key === 'up' ? -1 : 1)))
  ...
  if (key.ctrl && key.key === 'k') return replace(cursor, cursor === lineEnd ? Math.min(text.length, cursor + 1) : lineEnd)
  if (key.key === 'return' && (key.shift || key.meta)) return replace(cursor, cursor, '\n')
  if (!key.ctrl && !key.meta && !['return', 'tab', 'pageup', 'pagedown', 'escape', 'insert', 'capslock', 'numlock', 'scrolllock', 'pause', 'printscreen'].includes(key.key) && !/^f\d{1,2}$/.test(key.key)) {

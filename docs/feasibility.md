...
The 0.2.1 polish regression used Homebrew 2.1.280. It exposed gaps in the earlier validation: pasted spaces did not exercise physical `space` key events, and testing SDK cleanup did not establish that the rendered Close button discarded the chat. The fixes normalize named keys, explicitly dispose on every close route, identify submissions across editor remounts, and keep the composer in a stable layout so adding an error does not return focus to main. Live PTY verification covered physical Space, Shift+Enter, ordinary and enhanced Return, model selection with one visible label, command-error recovery and consecutive sends without refocusing, Close and corner × disposal, and blank reopening. Multiple Working and Thinking glyph frames were captured from real side-agent events. The spinner is drawn by the mod using supported Client timers; it is not an embedded copy of the parent's spinner. See `docs/evidence/polish-validation.json`. Cache measurements above remain from the original experiment.
